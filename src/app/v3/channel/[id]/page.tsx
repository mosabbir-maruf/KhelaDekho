...
export default function V3ChannelPage() {
  ...
  const selectQuality = useCallback((i: number) => setQualityIdx(i), []);

  return (
    <div className="min-h-screen">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-16 space-y-12">
        <PageHero icon={<Tv className="w-3 h-3 text-red-500" />} badge="V3 Stream" title={channel?.name || "Channel"} description={selectedUrl ? `Playing ${selectedUrl.label}` : "Admin-configured stream"} hint="Stream buffering? Try another quality." />
