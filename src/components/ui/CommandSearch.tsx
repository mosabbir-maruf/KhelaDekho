...
export function CommandSearch({ open, onClose }: CommandSearchProps) {
  ...
  const filtered = useMemo(() => {
    if (!query.trim()) return dynamicItems.length > 0 ? dynamicItems : defaultItems;
        ...
        <div className="border border-border-alt bg-page shadow-2xl shadow-black/50 overflow-hidden">
          ...
          <div className="flex items-center gap-3 px-4 py-3 border-b border-border">
            ...
            <input
              ...
              placeholder="Search live channels, matches, leagues..."
          ...
          <div className="max-h-[320px] overflow-y-auto py-2">
            {filtered.length === 0 ? (
              <div className="px-4 py-8 text-center">
                <p className="text-sm font-mono text-fg-dim">No matching indexes found.</p>
              </div>
            ) : (
              filtered.map((item, i) => (
                <button
